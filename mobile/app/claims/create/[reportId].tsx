...
export default function CreateClaimScreen() {
  ...
  return (
    <ScrollScreen>
      ...
      <Text className="mt-2 text-base text-slate-500">Berikan bukti yang jelas agar klaim mudah diverifikasi.</Text>

      {report ? (
        <View className="my-5 flex-row gap-4 rounded-3xl bg-white p-4 shadow-sm">
          <RemoteImage uri={report.image} className="h-20 w-20 rounded-2xl bg-slate-100" />
          <View className="flex-1 justify-center">
            <Text className="text-lg font-extrabold text-ink">{report.title}</Text>
            <Text className="mt-1 text-sm text-slate-500">{report.location}</Text>
          </View>
        </View>
      ) : null}
      ...
      <View className="gap-4 rounded-3xl bg-white p-5 shadow-sm">
        ...
        <ImageInput label="Tambahkan proof image opsional" value={proofImage} onChange={setProofImage} />
        ...
        <Pressable
          ...
          {loading ? (
            ...
            <>
              ...
              <Text className="text-base font-extrabold text-white">Submit Claim</Text>
            </>
          )}
        </Pressable>
      </View>
